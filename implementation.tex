...
Our task was to find the way of implementing a problem of image processing on the Android platform.
When implementing the software for Android we are limited to the input data of lower quality.
The Android camera lenses are made of plastic and causes a non-linear distortion.
Also, due to the limited capacity of Android calculation memory we need to down-sample the resolution of the input images.
In this implementation the expected size of an image is approximately 800 $\times$ 1000 pixels.
These problems avoid using common approaches of solving the dens corresponding problem.
The expected input data are described in Section \ref{prob}.
%In the first section of this chapter we enumerate the subtasks and chosen algorithms and then describe the structure of the software resources.
 
\section{Implementation outline}
In the first section of this chapter we describe the way how was the task solved, what algorithms were chosen and how was the application implemented.
